...
import {
  ...
  Plus, Trash2, Printer, Upload, Download, X, Check, AlertTriangle, Search,
...
const nextProductCode = (counter) => String(counter).padStart(4, "0");
...
const canAccessRegister = (user, moduleId, registerId) => {
  if (!user) return false;
  if (user.isAdmin) return true;
  if (!canAccess(user, moduleId)) return false;
  const regPerms = user.registerPermissions && user.registerPermissions[moduleId];
  if (regPerms === undefined) return true; // not configured yet -> full access within the module
  return !!regPerms[registerId];
};
...
async function ensureLocationsList() {
  ...
  if (!snap.exists()) await setDoc(LOCATIONS_DOC, { list: [{ id: "main", name: "Main Shop" }] });
}
...
/* ---------------------------------------------------------
   Image resize for logo upload (kept small so Firestore docs stay light)
--------------------------------------------------------- */
function resizeImageFile(file, maxWidth = 260) {
...
/* ---------------------------------------------------------
   Product picker — search, select, and it locks in (no more
   dropdown once chosen; delete the line to fix a mistake)
--------------------------------------------------------- */
    ...
    return (
      <div className="flex items-center justify-between gap-2">
        <div>
          ...
          <div className="text-[11px] text-slate-400 font-mono">{selected.sku} · In stock: {selected.stock}</div>
        </div>
        <button type="button" onClick={() => onChange("")} className="text-[11px] text-slate-400 hover:text-red-600 flex-shrink-0">Change</button>
      </div>
    );
  }

  const filtered = query.trim() === "" ? products.slice(0, 50) :
    products.filter((p) => (p.name + " " + p.sku + " " + p.category).toLowerCase().includes(query.toLowerCase())).slice(0, 50);
        ...
        <div className="absolute z-20 mt-1 w-full max-h-56 overflow-y-auto bg-white border border-slate-200 rounded-md shadow-lg">
          ...
          {filtered.map((p) => (
            <button key={p.id} type="button"
              ...
              <span className="text-slate-400 font-mono text-xs flex-shrink-0">stock: {p.stock}</span>
            </button>
          ))}
        </div>
      )}
...
function LineItemsEditor({ items, setItems, products, priceField, showCost }) {
  ...
  return (
    <div className="space-y-3">
      {items.map((line) => (
        <div key={line.id} className="border border-slate-200 rounded-md p-3 bg-white">
          <div className="flex items-start justify-between gap-2 mb-3">
            <div className="flex-1"><ProductPicker products={products} value={line.productId} onChange={(productId) => onProductChange(line.id, productId)} /></div>
            <button type="button" onClick={() => removeLine(line.id)} className="text-slate-400 hover:text-red-600 mt-0.5 flex-shrink-0" aria-label="Remove line"><Trash2 size={16} /></button>
          </div>
          <div className="grid grid-cols-3 gap-2">
            <Field label="Qty">
              <input type="number" inputMode="numeric" min="0" className={inputCls + " text-base"} value={line.qty}
                onChange={(e) => updateLine(line.id, { qty: Number(e.target.value) })} />
            </Field>
            <Field label={showCost ? "Unit Cost" : "Unit Price"}>
              <input type="number" inputMode="decimal" min="0" step="0.01" className={inputCls + " text-base"} value={line.price}
                onChange={(e) => updateLine(line.id, { price: Number(e.target.value) })} />
            </Field>
            <Field label="Line Total">
              <div className={inputCls + " bg-slate-50 text-right font-mono flex items-center justify-end"}>{money(line.qty * line.price)}</div>
            </Field>
          </div>
        </div>
      ))}
      {items.length === 0 && (
        <div className="text-center text-slate-400 text-sm py-6 border border-dashed border-slate-200 rounded-md">No items yet. Add a line below.</div>
      )}
    ...
    <div id="print-doc" className="hidden print:block p-8 text-black">
      <div className="flex justify-between items-start border-b-2 border-black pb-4 mb-4">
        <div className="flex items-center gap-3">
          {s.logoDataUrl && <img src={s.logoDataUrl} alt="" style={{ height: 48 }} />}
          <div>
...
function ReceiptDoc({ number, date, partyName, items, total, settings }) {
  const s = settings || {};
  return (
    <div id="print-receipt" className="p-1 text-center" style={{ width: "58mm" }}>
      {s.logoDataUrl && <img src={s.logoDataUrl} alt="" style={{ height: 32, margin: "0 auto 2px" }} />}
      <div className="font-bold text-[13px]">{s.shopName || "YOUR SHOP NAME"}</div>
      <div className="text-[10px] mb-1">Sales Receipt</div>
      <div className="text-[10px] font-mono border-t border-dashed border-black pt-1 text-left">{number}<br />{date}<br />{partyName ? `Customer: ${partyName}` : ""}</div>
      <div className="border-t border-dashed border-black mt-1 pt-1 text-left">
        {items.map((l, i) => (
      ...
      <div className="text-[9px] mt-2">{s.footerNote || "Thank you!"}</div>
    </div>
  );
}
...
/* ---------------------------------------------------------
   Stock module: Stock Levels + Goods Returns
--------------------------------------------------------- */
function StockLevels({ data, setData, save }) {
  ...
  const blank = { name: "", category: "", costPrice: 0, sellPrice: 0, stock: 0, minStock: 0 };
  ...
  const submit = (e) => {
    e.preventDefault();
    let products, counters = data.counters;
    if (editing) {
      products = data.products.map((p) => (p.id === editing ? { ...p, ...form, sku: p.sku, unit: "pc" } : p));
    } else {
      const sku = nextProductCode(data.counters.product);
      products = [...data.products, { ...form, sku, unit: "pc", id: uid(), createdAt: todayISO(), lastRestocked: todayISO() }];
      counters = { ...data.counters, product: data.counters.product + 1 };
    }
    const next = { ...data, products, counters };
  ...
  const downloadTemplate = () => {
    ...
    const ws = XLSX.utils.aoa_to_sheet([["Name", "Category", "Cost Price", "Sell Price", "Current Stock", "Minimum Stock"]]);
  ...
  const handleImport = async (e) => {
    ...
    try {
      ...
      let counter = data.counters.product;
      const imported = rows.map((r) => {
        const sku = nextProductCode(counter); counter++;
        return {
          id: uid(), sku, unit: "pc",
          ...
          stock: Number(norm(r, ["stock", "currentstock", "current stock", "qty", "quantity"])) || 0,
          minStock: Number(norm(r, ["minstock", "minimum stock", "min stock", "minimum", "reorder level"])) || 0,
          createdAt: todayISO(), lastRestocked: todayISO(),
        };
      });
      const next = { ...data, products: [...data.products, ...imported], counters: { ...data.counters, product: counter } };
      setData(next); save(next);
    } catch (err) {
      alert("Could not read that file. Use the downloadable template — columns: Name, Category, Cost Price, Sell Price, Current Stock, Minimum Stock.");
    }
    setImporting(false);
  ...
  return (
    <div>
      ...
      <p className="text-sm text-slate-500 mb-4">Stock levels, stock age, and reorder tags. Product codes are assigned automatically.</p>
      <div className="flex flex-wrap items-center gap-2 mb-4">
        ...
        <Btn variant="outline" onClick={downloadTemplate}><Download size={14} /> Download template</Btn>
      ...
      <div className="border border-slate-200 rounded-lg overflow-hidden bg-white">
        <table className="w-full text-sm">
          <thead className="bg-slate-50 text-[11px] font-mono uppercase tracking-wide text-slate-500">
            <tr>
              <th className="text-left px-3 py-2">Product</th><th className="text-left px-3 py-2">Code</th>
        ...
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
          <div className="bg-white rounded-lg w-full max-w-md p-5" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={submit} className="space-y-3">
              ...
              {!editing && <p className="text-xs text-slate-400">Product code will be assigned automatically ({nextProductCode(data.counters.product)}).</p>}
        ...
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
          <div className="bg-white rounded-lg w-full max-w-md p-5" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={submit} className="space-y-3">
              ...
              <Field label="Product"><ProductPicker products={data.products} value={form.productId} onChange={(id) => setForm({ ...form, productId: id })} /></Field>
...
function StockHub({ data, setData, save, user }) {
  ...
  useEffect(() => { if (allowedTabs.length && !allowedTabs.some((t) => t.id === tab)) setTab(allowedTabs[0].id); }, [allowedTabs.map((t) => t.id).join(",")]); // eslint-disable-line
  return (
    <div>
      <h1 className="text-xl font-semibold text-[#1F2428] mb-4">Stock</h1>
      {allowedTabs.length === 0 ? <NoRegistersNotice module="Stock" /> : (
        <>
...
/* ---------------------------------------------------------
   Quotation register — allowed to exceed stock (it's just a quote)
--------------------------------------------------------- */
function QuotationModule({ data, setData, save, setPrintJob, user }) {
  ...
  const convertToSalesOrder = (quote) => {
    ...
    if (insufficient.length > 0) {
      alert("Can't convert to a sales order — not enough stock for: " + insufficient.map((l) => {
        const p = data.products.find((p) => p.id === l.productId);
        return `${p.name} (have ${p.stock}, need ${l.qty})`;
      }).join(", ") + ". Adjust the quantities or wait for restock.");
      return;
    }
...
/* ---------------------------------------------------------
   Sales Order register — cannot exceed stock; cash or credit
--------------------------------------------------------- */
function SalesOrderModule({ data, setData, save, setPrintJob, user }) {
  ...
  const submit = (e) => {
    ...
    if (insufficient.length > 0) {
      alert("Not enough stock for: " + insufficient.map((l) => {
        const p = data.products.find((p) => p.id === l.productId);
        return `${p.name} (have ${p.stock}, need ${l.qty})`;
      }).join(", ") + ". A sales order can't sell more than what's in the system — use a Quotation instead if you need to quote ahead of stock.");
      return;
    }
    const number = nextNumber("SO", data.counters.sales);
...
/* ---------------------------------------------------------
   Cash Book row builder shared by the daily and running views
--------------------------------------------------------- */
function buildCashBookRows(data) {
  const dates = new Set([
    ...data.salesOrders.map((o) => o.date),
  ...
  return sorted.map((date) => {
    const cashSales = data.salesOrders.filter((o) => o.date === date && o.paymentType !== "credit").reduce((s, o) => s + o.total, 0);
    const creditSales = data.salesOrders.filter((o) => o.date === date && o.paymentType === "credit").reduce((s, o) => s + o.total, 0);
    const creditPayments = (data.payments || []).filter((p) => p.date === date);
    const creditPaymentsTotal = creditPayments.reduce((s, p) => s + p.amount, 0);
    const expenses = data.expenses.filter((e) => e.date === date).reduce((s, e) => s + e.amount, 0);
    const cashIn = cashSales + creditPaymentsTotal;
    const net = cashIn - expenses;
    running += net;
    return { date, cashSales, creditSales, creditPaymentsTotal, creditPayments, totalSales: cashSales + creditSales, expenses, net, cashInHand: running };
  }).reverse();
}

/* ---------------------------------------------------------
   Daily Cash Book (Sales) — resets to zero every day; the
   "Cash in Hand" figure is that day's activity only.
--------------------------------------------------------- */
function DailyCashBook({ data }) {
  const rows = useMemo(() => buildCashBookRows(data), [data.salesOrders, data.expenses, data.payments]);

  return (
    <div>
      <h2 className="text-lg font-semibold text-[#1F2428] mb-1">Daily Cash Book</h2>
      <p className="text-sm text-slate-500 mb-4">Starts fresh every day. Cash sales and credit payments actually received make up that day's cash in hand — credit sales are shown for reference only, until paid. For the running total across days, see Accounting → Reports.</p>
      <div className="border border-slate-200 rounded-lg overflow-hidden bg-white">
        <table className="w-full text-sm">
          <thead className="bg-slate-50 text-[11px] font-mono uppercase tracking-wide text-slate-500">
            <tr>
              ...
              <th className="text-right px-3 py-2">Credit Sales</th>
              <th className="text-right px-3 py-2">Credit Paid</th>
              <th className="text-right px-3 py-2">Total Sales</th>
              <th className="text-right px-3 py-2">Expenses</th>
              <th className="text-right px-3 py-2">Cash in Hand (day)</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r) => (
              <tr key={r.date} className="border-t border-slate-100 align-top">
                <td className="px-3 py-2">{r.date}</td>
                <td className="px-3 py-2 text-right font-mono text-emerald-700">{money(r.cashSales)}</td>
                <td className="px-3 py-2 text-right font-mono text-amber-700">{money(r.creditSales)}</td>
                <td className="px-3 py-2 text-right font-mono text-emerald-700">
                  {money(r.creditPaymentsTotal)}
                  {r.creditPayments.length > 0 && (
                    <div className="text-[10px] text-slate-400 font-mono normal-case mt-0.5">
                      {r.creditPayments.map((p) => <div key={p.id}>{p.salesOrderNumber} · {p.customer}</div>)}
                    </div>
                  )}
                </td>
                <td className="px-3 py-2 text-right font-mono">{money(r.totalSales)}</td>
                <td className="px-3 py-2 text-right font-mono text-red-600">{money(r.expenses)}</td>
                <td className={`px-3 py-2 text-right font-mono font-semibold ${r.net >= 0 ? "" : "text-red-600"}`}>{money(r.net)}</td>
              </tr>
            ))}
            {rows.length === 0 && <tr><td colSpan={7} className="text-center text-slate-400 py-8 text-sm">No activity recorded yet.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
...
/* ---------------------------------------------------------
   Running Cash Book (Accounting → Reports) — same format,
   but carries a cumulative "Cash in Hand" balance across days.
--------------------------------------------------------- */
function RunningCashBook({ data }) {
  const rows = useMemo(() => buildCashBookRows(data), [data.salesOrders, data.expenses, data.payments]);

  return (
    <div>
      <h2 className="text-lg font-semibold text-[#1F2428] mb-1">Cash Book — Running Total</h2>
      <p className="text-sm text-slate-500 mb-4">Same as the Daily Cash Book in Sales, but carrying a cumulative cash-in-hand balance across every day on record.</p>
      <div className="border border-slate-200 rounded-lg overflow-hidden bg-white">
        <table className="w-full text-sm">
          <thead className="bg-slate-50 text-[11px] font-mono uppercase tracking-wide text-slate-500">
            <tr>
              ...
              <th className="text-right px-3 py-2">Total Sales</th>
              <th className="text-right px-3 py-2">Expenses</th>
              <th className="text-right px-3 py-2">Net (day)</th>
              <th className="text-right px-3 py-2">Cash in Hand (running)</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r) => (
              <tr key={r.date} className="border-t border-slate-100 align-top">
                ...
                <td className="px-3 py-2 text-right font-mono text-emerald-700">{money(r.cashSales)}</td>
                <td className="px-3 py-2 text-right font-mono text-amber-700">{money(r.creditSales)}</td>
                <td className="px-3 py-2 text-right font-mono text-emerald-700">{money(r.creditPaymentsTotal)}</td>
                <td className="px-3 py-2 text-right font-mono">{money(r.totalSales)}</td>
                <td className="px-3 py-2 text-right font-mono text-red-600">{money(r.expenses)}</td>
            ...
            {rows.length === 0 && <tr><td colSpan={8} className="text-center text-slate-400 py-8 text-sm">No activity recorded yet.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
...
function DailyPurchaseReport({ data }) {
  const rows = useMemo(() => {
    ...
    data.purchaseOrders.forEach((o) => {
      if (!map[o.date]) map[o.date] = { date: o.date, total: 0, count: 0 };
  ...
  return (
    <div>
      ...
      <div className="border border-slate-200 rounded-lg overflow-hidden bg-white">
        <table className="w-full text-sm">
          <thead className="bg-slate-50 text-[11px] font-mono uppercase tracking-wide text-slate-500">
            <tr><th className="text-left px-3 py-2">Product</th><th className="text-left px-3 py-2">Code</th><th className="text-right px-3 py-2">Last Cost</th><th className="text-right px-3 py-2">Stock Qty</th><th className="text-right px-3 py-2">Stock Value</th></tr>
          </thead>
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} className="border-t border-slate-100">
...
/* ---------------------------------------------------------
   Creditors Report — overall, invoice-level (Accounting)
--------------------------------------------------------- */
  ...
  return (
    <div className="space-y-6">
      ...
      <DocList title="Purchase Orders" docs={data.purchaseOrders} columns={["Number", "Date", "Supplier", "Total", "Status"]}
        renderTag={(d) => <Tag tone={d.status === "received" ? "good" : "pending"}>{d.status}</Tag>} />
      <RunningCashBook data={data} />
    </div>
  );
}

/* ---------------------------------------------------------
   Sales module (hub with sub-registers) — filtered by rights
--------------------------------------------------------- */
function SalesHub({ data, setData, save, setPrintJob, user }) {
  const allowedTabs = REGISTERS.sales.filter((t) => canAccessRegister(user, "sales", t.id));
  const [tab, setTab] = useState(allowedTabs[0] ? allowedTabs[0].id : "orders");
  useEffect(() => { if (allowedTabs.length && !allowedTabs.some((t) => t.id === tab)) setTab(allowedTabs[0].id); }, [allowedTabs.map((t) => t.id).join(",")]); // eslint-disable-line
  return (
    <div>
      <h1 className="text-xl font-semibold text-[#1F2428] mb-4">Sales</h1>
      {allowedTabs.length === 0 ? <NoRegistersNotice module="Sales" /> : (
        <>
          <TabBar tabs={allowedTabs} active={tab} onChange={setTab} />
...
function PurchaseHub({ data, setData, save, setPrintJob, user }) {
  const allowedTabs = REGISTERS.purchase.filter((t) => canAccessRegister(user, "purchase", t.id));
  const [tab, setTab] = useState(allowedTabs[0] ? allowedTabs[0].id : "orders");
  useEffect(() => { if (allowedTabs.length && !allowedTabs.some((t) => t.id === tab)) setTab(allowedTabs[0].id); }, [allowedTabs.map((t) => t.id).join(",")]); // eslint-disable-line
  return (
    <div>
      <h1 className="text-xl font-semibold text-[#1F2428] mb-4">Purchase</h1>
      {allowedTabs.length === 0 ? <NoRegistersNotice module="Purchase" /> : (
        <>
          <TabBar tabs={allowedTabs} active={tab} onChange={setTab} />
...
--------------------------------------------------------- */
function AccountingHub({ data, user }) {
  const allowedTabs = REGISTERS.accounting.filter((t) => canAccessRegister(user, "accounting", t.id));
  const [tab, setTab] = useState(allowedTabs[0] ? allowedTabs[0].id : "costing");
  useEffect(() => { if (allowedTabs.length && !allowedTabs.some((t) => t.id === tab)) setTab(allowedTabs[0].id); }, [allowedTabs.map((t) => t.id).join(",")]); // eslint-disable-line
  return (
    <div>
      <h1 className="text-xl font-semibold text-[#1F2428] mb-4">Accounting</h1>
      {allowedTabs.length === 0 ? <NoRegistersNotice module="Accounting" /> : (
        <>
          <TabBar tabs={allowedTabs} active={tab} onChange={setTab} />
...
/* ---------------------------------------------------------
   Settings: Users & Rights (module + register level)
--------------------------------------------------------- */
function UsersSettings({ users, refreshUsers, locations }) {
  const [showForm, setShowForm] = useState(false);
  const [editingUser, setEditingUser] = useState(null);
  const blank = { name: "", username: "", password: "", permissions: {}, registerPermissions: {}, locationIds: [] };
  const [form, setForm] = useState(blank);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");

  const openNew = () => { setForm(blank); setEditingUser(null); setError(""); setShowForm(true); };
  const openEdit = (u) => { setForm({ name: u.name, username: u.username, password: "", permissions: u.permissions || {}, registerPermissions: u.registerPermissions || {}, locationIds: u.locationIds || [] }); setEditingUser(u); setError(""); setShowForm(true); };

  const toggleModule = (moduleId) => {
    const turningOn = !form.permissions[moduleId];
    let regPerms = form.registerPermissions;
    if (turningOn && REGISTERS[moduleId] && !regPerms[moduleId]) {
      const allOn = {};
      REGISTERS[moduleId].forEach((r) => { allOn[r.id] = true; });
      regPerms = { ...regPerms, [moduleId]: allOn };
    }
    setForm({ ...form, permissions: { ...form.permissions, [moduleId]: turningOn }, registerPermissions: regPerms });
  };
  const toggleRegister = (moduleId, regId) => {
    const current = form.registerPermissions[moduleId] || {};
    setForm({ ...form, registerPermissions: { ...form.registerPermissions, [moduleId]: { ...current, [regId]: !current[regId] } } });
  };
    ...
    try {
      if (editingUser) {
        await setDoc(doc(db, "users", editingUser.id), {
          name: form.name, username: editingUser.username, isAdmin: false,
          permissions: form.permissions, registerPermissions: form.registerPermissions, locationIds: form.locationIds,
        });
      } else {
        ...
        await setDoc(doc(db, "users", cred.user.uid), {
          name: form.name, username: form.username.trim().toLowerCase(), isAdmin: false,
          permissions: form.permissions, registerPermissions: form.registerPermissions, locationIds: form.locationIds,
        });
  ...
  return (
    <div>
      ...
      <p className="text-sm text-slate-500 mb-4">New team members can create their own login on the sign-in screen — they'll see nothing until you grant modules, registers, and a location below.</p>
      <div className="border border-slate-200 rounded-lg overflow-hidden bg-white">
        <table className="w-full text-sm">
          <thead className="bg-slate-50 text-[11px] font-mono uppercase tracking-wide text-slate-500">
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
          <div className="bg-white rounded-lg w-full max-w-lg p-5 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-4">
            ...
            <form onSubmit={submit} className="space-y-3">
              ...
              <Field label="Modules & registers this person can access">
                <div className="space-y-2 mt-1">
                  {MODULES.map((m) => (
                    <div key={m.id} className="border border-slate-200 rounded-md p-2">
                      <label className="flex items-center gap-2 text-sm font-medium cursor-pointer">
                        <input type="checkbox" checked={!!form.permissions[m.id]} onChange={() => toggleModule(m.id)} />
                        {m.label}
                      </label>
                      {form.permissions[m.id] && REGISTERS[m.id] && (
                        <div className="grid grid-cols-2 gap-1 mt-2 pl-5">
                          {REGISTERS[m.id].map((r) => (
                            <label key={r.id} className="flex items-center gap-1.5 text-xs text-slate-600 cursor-pointer">
                              <input type="checkbox" checked={!!(form.registerPermissions[m.id] && form.registerPermissions[m.id][r.id])} onChange={() => toggleRegister(m.id, r.id)} />
                              {r.label}
                            </label>
                          ))}
                        </div>
...
/* ---------------------------------------------------------
   Settings: Invoice Layout (logo, letterhead, for the active location)
--------------------------------------------------------- */
function InvoiceLayoutSettings({ data, setData, save, activeLocationName }) {
  const [form, setForm] = useState(data.invoiceSettings);
    ...
    <div>
      ...
      <form onSubmit={submit} className="space-y-3 max-w-md">
        <Field label="Company logo">
          <div className="flex items-center gap-3">
            {form.logoDataUrl && <img src={form.logoDataUrl} alt="Logo" className="h-12 border border-slate-200 rounded bg-white p-1" />}
            <input type="file" accept="image/*" onChange={handleLogoUpload} disabled={logoBusy} className="text-sm" />
            {logoBusy && <Loader2 size={14} className="animate-spin text-slate-400" />}
            {form.logoDataUrl && <button type="button" onClick={() => setForm({ ...form, logoDataUrl: "" })} className="text-xs text-red-600">Remove</button>}
          </div>
        </Field>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(locationDocRef(activeLocationId), (snap) => {
      if (snap.exists()) setData(mergeWithDefaults(snap.data()));
    });
    return unsub;
  }, [activeLocationId]);
  ...
  const handleCreateAdmin = async ({ name, username, password }) => {
    ...
    try {
      ...
      await setDoc(doc(db, "users", cred.user.uid), { name, username: username.trim().toLowerCase(), isAdmin: true, permissions: {}, registerPermissions: {}, locationIds: [] });
  ...
  const handleSignUp = async ({ name, username, password }) => {
    ...
    try {
      const cred = await createUserWithEmailAndPassword(auth, toAuthEmail(username), password);
      await setDoc(doc(db, "users", cred.user.uid), { name, username: username.trim().toLowerCase(), isAdmin: false, permissions: {}, registerPermissions: {}, locationIds: [] });
    } finally { setAuthBusy(false); }
  };
  ...
  return (
    <div className="min-h-screen bg-[#F4F4F2] text-[#1F2428] flex" style={{ fontFamily: "Inter, system-ui, sans-serif" }}>
      ...
      <main className="flex-1 p-5 md:p-8 pb-20 md:pb-8 max-w-6xl print:hidden">
        ...
        {activeView === "accounting" && <AccountingHub data={data} user={currentUser} />}
