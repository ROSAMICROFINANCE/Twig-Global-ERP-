          ...
          {items.map((line) => (
            <tr key={line.id} className="border-t border-slate-100">
              <td className="px-2 py-1.5 min-w-[220px]">
                <ProductPicker products={products} value={line.productId} onChange={(productId) => onProductChange(line.id, productId)} />
              </td>
...
/* ---------------------------------------------------------
   Daily Cash Book — real cash flow: cash sales + credit
   payments actually received, minus expenses.
--------------------------------------------------------- */
function DailyCashBook({ data }) {
  ...
  }, [data.salesOrders, data.expenses, data.payments]);

  return (
    <div>
      <h2 className="text-lg font-semibold text-[#1F2428] mb-1">Daily Cash Book</h2>
      <p className="text-sm text-slate-500 mb-4">Cash sales and credit payments actually received make up real cash in hand. Credit sales are shown for reference only, until paid.</p>
      <div className="border border-slate-200 rounded-lg overflow-hidden bg-white">
        <table className="w-full text-sm">
          <thead className="bg-slate-50 text-[11px] font-mono uppercase tracking-wide text-slate-500">
            <tr>
